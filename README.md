...
> 协作式 Local 并发、显式 Worktree 隔离、Git 审阅、三方结果应用和崩溃恢复已实现；浏览器/OCR、复杂文档等通用 Changes、
...
- Git 项目按 Run 和工具调用记录归属明确的变更；每轮回答末尾汇总已编辑文件和增删行，可展开文件、
  撤销整轮或打开右侧 Changes 审核真实 Diff。满足文件未被后续任务继续修改、`HEAD` 与暂存区未变化等
  条件时，可安全撤回该 Run 的文件和会话消息。
- 普通并发任务默认共享 Local，并通过工作区 revision、文件版本校验、读写协调和变化通知避免陈旧覆盖；
  不会因为开启第二个任务或 Agent 判断风险而自动创建 Worktree。
- 用户可在任务标题栏显式创建或选择 Worktree；该任务的主 Agent、子 Agent 和工具共享同一隔离目录。
  Lumora 新建的临时 Worktree 结束后可在 Changes 审阅累计修改并选择三方应用到 Local、创建正式
  分支或放弃，也可主动开启“无冲突时自动应用”。用户已有的 linked Worktree 作为非托管正式环境

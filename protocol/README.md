# LUMORA Protocol

LUMORA 跨进程接口的唯一协议源。

## 职责

- 定义 Java Local Core 与 Python Agent Runtime 的 gRPC 接口。
- 生成 Java 和 Python 强类型代码。
- 检查协议格式和破坏性变更。

协议包使用 `lumora.v1` 命名空间。运行时不得共享业务源码，只能依赖生成的协议代码。
Java 代码由 `core/pom.xml` 中固定版本的 Protobuf Maven Plugin 直接从本目录生成，
避免 Maven 与 Buf 同时生成两套 Java 类。

Electron Main 与 Java 使用 REST + SSE，REST DTO 分别在 TypeScript 和 Java 工程中维护，
不从 Protobuf 生成。Renderer 仍然只能通过 Preload 白名单 IPC 访问后端能力。

## 目录

```text
proto/lumora/v1/common.proto  公共认证、健康检查和错误
proto/lumora/v1/core.proto    Java/Python 共享的任务、事件和审批消息
proto/lumora/v1/agent.proto   规划消息和 AgentService
```

## 检查

不依赖外部工具的结构检查：

```powershell
powershell -ExecutionPolicy Bypass -File protocol/tests/contract-shape.ps1
```

配置 Buf CLI 后执行完整 Lint，并生成 Python 代码：

```powershell
powershell -ExecutionPolicy Bypass -File protocol/generate.ps1
```

...
`lumora-cloud` 的流式 `usage` 是云端累计计费快照，可能包含托管检索或供应商内部阶段性工作，不能
直接作为执行中的活动上下文锚点。Cloud Provider 缓存这些中间快照，只在 `completed` 时采用最终
Usage（终态缺失时才回退到最后一个快照）；详细计费字段保持不变，Web Search 仅对用于上下文圆环的
`promptTokens` 做本地请求投影校正。工具结果加入后产生的混合估算仍用于内部压缩判断，但云端界面
继续显示上一次已结算上下文，直到下一次模型 `completed` 返回真实锚点。三个本地 BYOK Provider 的
流式 usage 行为不变。

Desktop 圆环与右侧总量共用独立的 Renderer 上下文快照，不再把实时回复文字加到锚点上。
前端利用 `protocol_message` 的 assistant 消息识别模型回合结束，只在随后的非临时 `usage`
到达时更新快照；工具结果后的混合估算、临时 `usageProvisional`、阶段回复转入工作日志和普通
历史刷新不改变已结算快照。自动/手动压缩完成后可立即更新，真实的新请求上下文也允许下降。
缓存任务保留运行中的快照与结算边界，重新加载/切换分支从对应历史初始化；没有上下文锚点时
才使用本地文本估算，不回退到累计计费 `TokenUsage`。本地与云端共用这层显示策略，后端
Provider、执行、压缩判定和累计计费均不受影响。由于来源精度标志尚未提供，界面继续显示“约”。

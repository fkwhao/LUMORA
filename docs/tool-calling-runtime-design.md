...
用户输入
  ...
  → [5] 自动 Reviewer：通过则执行，其他结果阻止并要求主 Agent 更换方案
  → [6] HITL：仅“请求审批”模式暂停 Agent Loop 等待用户
  → 工具执行
```
...
- `auto_approve`：只读工具和工作区内的新文件创建默认允许；覆盖已有文件、局部补丁、
  Shell 等需要判断的调用交给自动 Reviewer。Reviewer 只能允许当前一次；`deny`、
...
“替我审批”不依赖第三方 Agent SDK。Python 运行时用当前任务已经选择的 OpenAI 兼容
API、API Key 和模型额外发起一个无工具的 Reviewer 回合，只提交用户当前请求、主 Agent
...
- 文件路径解析后验证工作区边界；外部路径在替我审批模式下不执行，其他模式只有当前
  单次审批通过才能执行。
- Shell 灾难性黑名单只用于不可接受的绝对拒绝，其余风险交给规则和权限模式；HITL 只
  服务于人工审批模式。

...
- 本地文本页签打开完整文件。页签下方使用内联引用专属的紧凑面包屑路径栏，从项目目录名开始并以向右箭头区分目录层级；箭头左右留白对称，所有路径层级与最终文件名使用相同字号、字重和前景色。路径过长时可以水平滚动，不改变 Changes/Diff 的文件头样式。即使引用的是项目根目录文件，也至少显示“项目 > 文件”两个层级。
- 代码和常见结构化文本复用只读源码画布、行号栏、滚动条和语法高亮，并将 `#Lx-Ly` 自动定位到可视区域中央。只读源码的行号贴近面板左边界，行号栏按当前文件最大行号的位数自动收缩，使正文紧随行号且不会挤压多位数字；行号与正文之间不绘制竖线。引用行仅在代码区域显示中性浅蓝底色，行号只改变文字颜色，不显示红绿增删背景或 `+/-` 标记。
- Markdown 默认按主聊天界面的统一 Markdown 组件渲染，并提供“查看源代码 / 预览”切换；图片和已登记的 PDF 附件继续使用内嵌预览，PDF 按 `#Px-Py` 的起始页打开。
...
- 文本和图片预览设置大小上限；允许范围内的本地文本一次读取完整文件，不再静默返回截断前缀，超过上限则明确拒绝预览。未知二进制格式不直接读取到 Renderer。

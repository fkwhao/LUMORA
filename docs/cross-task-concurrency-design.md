...
并发。Run 事件默认以 20ms 短窗口按 Run 合并落库：单个公开事件和序号仍完整保留，
但一批只做一次 Run 读取、一次事务和一次序号更新；终态与生命周期边界同步刷盘。
每个连接同时启用 10 秒 `busy_timeout`，吸收进程外的瞬时文件锁。WAL 负责持久日志
模式，单连接池和事件写入日志负责进程内写入仲裁，避免把正常写竞争暴露成
`SQLITE_BUSY`。
